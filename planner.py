...
from pybullet_planning import add_data_path, connect, dump_body, disconnect, wait_for_user, \
    ...
    assign_link_colors, add_line, point_from_pose, remove_handles, BLUE, INF, PANDA_INFO, FRANKA_URDF, get_ik_joints, either_inverse_kinematics, check_ik_solver
...
def main():
    ...
    plane =  p.loadURDF(os.path.join(urdfRootPath,"plane.urdf"), basePosition=[0,0,-0.65])
    
    
    with LockRenderer():
        with HideOutput(True):
            urdf_file = os.path.join('pybullet_planning', FRANKA_URDF)
            robot = load_pybullet(urdf_file, fixed_base=True)
