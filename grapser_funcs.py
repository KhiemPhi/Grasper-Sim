...
def reset_hand(rID=None, rPos=(0, 0, -init_grasp_distance), rOr=(0, 0, 0, 1), fixed=True):
    ...
    rID = p.loadURDF(os.path.join(urdfRootPath, "franka_panda/panda.urdf"),useFixedBase=True)
    if debug_lines:
        add_debug_lines(rID)
...
def reset_ob(oID=None, oPos=(0, 0, 0), fixed=True):
    ...
    models = md.model_lib()
    state_object= [0.6,0,0.1]
    oID = p.loadURDF(models['mug'], basePosition=state_object)
